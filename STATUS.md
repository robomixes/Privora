...
- **Ask My Documents Phase 0 quality gate** — see `~/.claude/plans/starry-orbiting-galaxy.md`. Run 5 docs × 3 difficulty tiers (extractive / inferential / interpretive). Decision criterion: ≥80% easy correct AND ≤20% hard confidently-wrong → greenlight Phase 1 (embedder + chunking + vector store + citations). Below threshold = small-model quality is the bottleneck; stop and revisit with future Gemma versions.
