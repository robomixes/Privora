...
### Track A — ML Kit removal, phased toward F-Droid main eligibility (IN PROGRESS — 4/5 deps removed as of 2026-05-20)
...
| **3** | Translation — flavor-gated. `playstore` keeps `mlkit-translate`. `fdroid` ships **Option A** (Gemma-only translation via the existing path at [`TranslateScreen.kt:379-395`](app/src/main/java/com/privateai/camera/ui/translate/TranslateScreen.kt#L379-L395), 1-2 days) as the recommended start. Option B (Argos Translate, 2-4 weeks) is a separate workstream if Option A doesn't satisfy users. | ⏳ NOT STARTED | — |

**Status as of 2026-05-20**: 4 of 5 ML Kit deps removed from BOTH flavors (`barcode-scanning`, `face-detection`, `text-recognition`, `play-services-mlkit-document-scanner`). Only `mlkit-translate` remains — Phase 3 will flavor-gate it (kept in `playstore`, replaced with Gemma in `fdroid`). After A3 lands the `fdroid` flavor is genuinely ML-Kit-free and ready for the F-Droid main resubmission (subject to Gemma's "Gemma Terms of Use" license review by F-Droid maintainers).
...
|---------|----------|--------|
| **Track A3 — Translation flavor-gating (Option A: Gemma-only on fdroid)** | High | ~1-2 days. ONLY remaining ML Kit dep. After this, zero ML Kit in fdroid flavor → F-Droid main eligibility unblocked. |
...
└── Dependencies
    ...
    ├── ML Kit translate ONLY (others removed via Track A: barcode→ZXing, face→ONNX YuNet, OCR→Tesseract, scanner→CameraX+manual crop)
